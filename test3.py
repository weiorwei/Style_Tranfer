...
vgg19 = torchvision.models.vgg19(weights=weight)  # 加载vgg19 模型
vgg19.eval()
print(vgg19)
...
if not os.path.exists(save_path):
    os.mkdir(save_path)
STYLE_Weight_LAYERS = [(0.5, 1), (1.0, 5), (1.5, 10), (3.0, 19), (4.0, 34)]  # 选择计算gram矩阵的卷积层，以及对应的权重
Weight_LAYERS=0
CONTEN_LAYERS=12
# ------------------------------
...
# 获得模型的每一层输出结果，输出结果为一个列表  例： pre[1]为第一层的输出
...
def content_loss(origin_img, genera_img, compare_layer):
    pre_orig = pick_conv(origin_img, vgg19, compare_layer)  # 将原始图片输入模型获得每一层的输出
    pre_genera_img = pick_conv(genera_img, vgg19, compare_layer)  # 将生成的图片图片输入模型获得每一层的输出
    x = pre_orig[
    ...
    m = (x - y).numpy()
    content_loss = (1 / (4 * area * height)) * np.sum(m ** 2)
...
def gram_maxtrix(img):
    img = img.squeeze(0)
    maxtrix = np.reshape(img, (img.shape[0], img.shape[1] * img.shape[2]))
    return (np.matmul(np.transpose(maxtrix), maxtrix)).numpy()
...
#         maxtrix = np.reshape(pre, (H, area))  # 将三维矩阵转换成2维矩阵
#         return np.matmul(np.transpose(maxtrix), maxtrix)  # 计算gram矩阵，两个矩阵转置相乘
#
#     pre_style = pick_conv(style_img, vgg19, 36)  # 将原始图片输入模型获得每一层的输出
#     pre_genera_img = pick_conv(genera_img, vgg19, 36)  # 将生成的图片图片输入模型获得每一层的输出
...
#         b = a ** 2
#         b = b.numpy()
#         _style_loss += w * (1 / (4 * area1 ** 2 * H1 ** 2) * np.sum(b))  # np.sum() 必须是numpy的数据
...
class content(nn.Module):
    def __init__(self, target_img):
        ...
        self.target = pick_conv(target_img.to(torch.float32), vgg19, 20)[19].detach()

    def forward(self, gen_img):
        gen_con = pick_conv(gen_img.to(torch.float32), vgg19, 20)[19]
        x = F.mse_loss(self.target, gen_con)
        return x
...
class style(nn.Module):
    def __init__(self, target_img):
        ...
        pre = pick_conv(target_img.to(torch.float32), vgg19, 36)
        self.target=[]
        for w, layer in Weight_LAYERS:
            self.target.append(gram_maxtrix(pre[layer]))

    def forward(self, gen_img):
        pre = pick_conv(gen_img.to(torch.float32), vgg19, 36)
        gen_gram = []
        for w, layer in Weight_LAYERS:
            gen_gram.append(gram_maxtrix(pre[layer]))
        z = 0
        for m in range(len(self.target)):
            z += F.mse_loss(torch.from_numpy(self.target[m]).to(torch.float32), torch.from_numpy(gen_gram[m]).to(torch.float32))
        return z
...
# input_c = input.clone()
optimizer = optim.Adam([input],lr=0.0001)  # 选择优化器
# 实例化两个损失函数
loss_content_class = content(content_img)
loss_style_class = style(style_img)

for i in range(epoch):
    optimizer.zero_grad()

    loss_style = loss_style_class(input)  # 风格损失
    loss_content = loss_content_class(input)  # 内容损失
