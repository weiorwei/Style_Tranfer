...
#计算图片的平均值
image = imageio.imread(CONTENT_IMG)
image = resize(image, (IMAGE_H, IMAGE_W), preserve_range=True)
MEAN_R=np.mean(image[:,:,0])
MEAN_G=np.mean(image[:,:,1])
MEAN_B=np.mean(image[:,:,2])
MEAN_VALUES = np.array([MEAN_R, MEAN_G, MEAN_B]).reshape((1, 1, 1, 3))
...
def load_image(path):
    ...
    image = resize(image, (IMAGE_H, IMAGE_W), preserve_range=True)
    image = np.reshape(image, image.shape).astype('uint8')
...
with tf.compat.v1.Session() as sess:
    ...
    optimizer = tf.compat.v1.train.AdamOptimizer(0.25)
